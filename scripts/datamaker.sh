...
do
    ...
    timeEntered="$(date +%F) $(shuf -i 1-24 -n 1):$(shuf -i 1-60 -n 1):$(shuf -i 1-60 -n 1)"
    ...
    echo "INSERT INTO log (logTimeCreated, logTimeEdited, loginAttemptId, studentId) values ($timeEntered, $timeEntered, $attemptId, $id);" > temp.sql
    mysql --user=$user --password=$pass $db < temp.sql > /dev/null
